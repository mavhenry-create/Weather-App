...
function debounce(fn, delay = 300) { // debounce function to limit the rate of API calls while typing in the search input
  let timerId;
...
const API_KEY = "u32zitr7vit34p2eyrv2u0dj79shhnq2mn63yxjd"; // Hide this key in production
const search = searchInput.value.trim(); // Default to the input value, can be updated on button click
const BASE_URL = "https://www.meteosource.com/api/v1/free";

// Function to construct the URL for finding places based on user input
const findPlaceUrl = (search) => {
  return `${BASE_URL}/find_places?text=${encodeURIComponent(search)}&key=${API_KEY}`;
};

const findPlacePrefixUrl = (search) => {
  return `${BASE_URL}/find_places_prefix?text=${encodeURIComponent(search)}&key=${API_KEY}`;
};

// Function to construct the URL for fetching weather data based on place ID
const weatherDataUrl = (placeId) => {
  ...
  return `${BASE_URL}/point?${params.toString()}`;
};
...
// If a place has been selected from the suggestions, use its place ID to fetch weather data. Otherwise, find the place based on the search input and then fetch weather data for the first matching result.
  try {
    ...
    let selectedPlace;
    if (selectedPlaceId) {
      const weatherRes = await fetch(weatherDataUrl(selectedPlaceId)); // When a suggestion is selected, use the cached place ID to fetch weather data directly without needing to find the place again
      if (!weatherRes.ok) {
        throw new Error(
      ...
      const placeRes = await fetch(findPlaceUrl(search));
      if (!placeRes.ok) {
      ...
      const weatherRes = await fetch(weatherDataUrl(selectedPlace.place_id));
      if (!weatherRes.ok) {
        throw new Error(
          `Failed fetching weather data status: ${weatherRes.status}`,
        );
...
async function fetchSuggestions(query) {
  ...
  try {
    const resAutoComplete = await fetch(findPlacePrefixUrl(query));
    ...
    return suggestions.map((item) => { // Construct the label for each suggestion by combining available details (name, region, country) while filtering out any empty values to avoid extra commas
      const labelParts = [item.name, item.adm_area1, item.country].filter(
        Boolean,
      );
      const label = labelParts.join(", ");

      return { // Return an object for each suggestion containing the label for display and the place details for quick access when a suggestion is clicked
        label: label,
...
async function renderSuggestions(options) { // Render the autocomplete suggestions in the datalist element and store the place details in a map for quick access when a suggestion is clicked
...
async function displayWeather() { // Main function to display weather data in the UI by fetching it based on user input or selected suggestion and then updating the relevant DOM elements with the received data
  ...
  const {day = "", all_day = {}, summary: dailySummary = ""} = todayDaily || {};
  ...
  document.getElementById("temperature").textContent = `Temperature: ${temperature}${tempUnit}`;
  document.getElementById("temphigh").textContent = `High: ${temperature_max}${tempUnit}`;
  document.getElementById("templow").textContent = `Low: ${temperature_min}${tempUnit}`;
  document.getElementById("daily-summary").textContent = `Summary: ${dailySummary}`;
  document.getElementById("summary").textContent = `Condition: ${summary}`;
  document.getElementById("cloud-cover").textContent =
    `Cloud Cover: ${cloud_cover}%`;
  document.getElementById("wind").textContent = `Wind: ${dir} ${speed} ${windUnit}`;
...
themeToggleBtn.addEventListener("click", () => {
    document.getElementById('theme-toggle-btn').textContent = document.documentElement.classList.contains("dark")
    ? "🌑"
    : "🌕";
